...
如果把这 13 篇论文压成几条主线，我会分成五类：

1. **经验表示进化**：关注“经验对象应该长什么样”，代表是 Strategy Genes。
2. **Prompt Optimization / Textual Gradient**：关注“如何把模型反馈变成文字版梯度，再自动改写 prompt”，代表是 APO、FedTextGrad。
3. **技能资产进化**：关注“怎样把经验沉淀成 skill，并让 skill 持续运维”，代表是 MUSE、SkillForge、SkillX、Trace2Skill、SkillClaw、Skill1。
4. **训练数据进化**：关注“训练集能否跟着 Agent 的弱点一起变”，代表是 CoEvolve。
5. **Harness / 协议进化**：关注“真正该进化的不只是提示词或 skill，还包括 tool、memory、middleware、agent code 乃至资源协议”，代表是 Autogenesis、AHE、Continual Harness。

如果只看一条连续谱，我会这样概括：

- **Strategy Genes** 在回答“经验对象应如何表示”。
- **APO / FedTextGrad** 在回答“prompt 能否像参数一样被迭代更新，而且更新信号本身可以是文本”。
...
### 2.2 Prompt Optimization / Textual Gradient

```mermaid
flowchart TD
    A[当前 Prompt] --> B[在样本上执行]
    B --> C[生成自然语言批评]
    C --> D[按批评方向改写 Prompt]
    D --> E[评估候选 Prompt]
    E --> F[保留更优版本]
    F --> A
    F --> G[联邦场景：客户端提交本地 Prompt 更新]
    G --> H[服务器汇总文本更新]
    H --> A
```

这条路线最看重的是：不依赖数值梯度，而是把语言反馈本身做成可迭代的优化信号。
...
| [Automatic Prompt Optimization with "Gradient Descent" and Beam Search](https://arxiv.org/abs/2305.03495) | 文本梯度式 prompt 优化起点 | prompt 文本与候选改写路径 | minibatch 错误样本、自然语言批评、beam search 评分 | 情感分类、自然语言推断、越狱检测 |
| [Can Textual Gradient Work in Federated Learning?](https://arxiv.org/abs/2502.19980) | 联邦 textual gradient 框架 | 多客户端本地 prompt 更新与服务器聚合结果 | 客户端 textual gradients、服务器文本汇总、UID 引导摘要 | 联邦 prompt 优化实验设置 |
...
### 4.2 Automatic Prompt Optimization with "Gradient Descent" and Beam Search

链接：[arXiv 2305.03495](https://arxiv.org/abs/2305.03495)  
版本：v1，2023-05-04

**一句话**：APO 的核心想法是，不去真的算数值梯度，而是让模型写出“这段 prompt 哪里写得不好”的文字批评，再据此自动改写 prompt。

**论文要解决什么问题**

- 很多 prompt 之所以有效，靠的是人工反复试错。
- 这种方式有两个明显问题：
  - 成本高，改一条 prompt 往往要来回试很多次；
  - 不稳定，不同人写出来的 prompt 风格和质量差异很大。
- 作者想解决的是：能不能像做梯度下降那样，把 prompt 优化变成一个自动、迭代、数据驱动的过程。

**核心方法**

- 先拿一个当前 prompt，在一小批训练样本上跑出结果。
- 再让 LLM 根据错误案例写出一段 **自然语言批评**，指出 prompt 模糊、遗漏或容易误导的地方。
- 这段批评就被视为“文本梯度”：
  - 它不是数值向量；
  - 它更像“老师给作文写的修改意见”。
- 然后再让 LLM 按照与这段批评“相反的语义方向”去改 prompt。
  - 如果批评说“定义太模糊”，就把 prompt 改得更具体；
  - 如果批评说“类别边界不清楚”，就补充更明确的判别规则。
- 为了避免每次只赌一个改写版本，论文又加了两层搜索：
  - `beam search`：同时保留多条候选改写路线；
  - `bandit selection`：把评估预算更多分给更有希望的候选。

**实验结论**

- 论文在三个 benchmark NLP 任务和一个 jailbreak detection 任务上评测。
- APO 相比更早的 prompt editing 方法表现更好。
- 论文给出的代表性结果是：相对初始 prompt，性能最高可提升 **31%**。
- 作者特别强调，APO 经常把“含糊任务描述”改写成“更像标注规范的精确指令”，这正是它有效的关键。

**我怎么理解这篇论文**

- APO 的价值，不只是“prompt 自动调参”，而是它第一次把“文本反馈也能充当梯度”这件事讲清楚了。
- 它告诉我们：如果问题本身没有可微 loss，也不代表没法做优化；只是优化信号从数字变成了文字。
- 在今天回头看，很多 textual gradient、self-refine、prompt optimizer 工作，都能在这篇里找到雏形。

### 4.3 Can Textual Gradient Work in Federated Learning?

链接：[arXiv 2502.19980](https://arxiv.org/abs/2502.19980)  
版本：v1，2025-02-27

**一句话**：FedTextGrad 想回答的是，既然 prompt 可以靠“文字版梯度”更新，那这种更新能不能像联邦学习里的参数梯度一样，被多客户端分布式汇总。

**论文要解决什么问题**

- APO、TextGrad 这类方法通常默认：
  - 数据在一个地方；
  - prompt 在一个地方优化；
  - 汇总的是单机上的反馈。
- 但联邦学习的现实场景不是这样：
  ...
  - 客户端不能直接上传原始数据；
  - 服务器需要聚合多个本地更新结果。
- 传统 FL 聚合的是数值梯度或参数更新，而 textual gradient 是文本。作者要解决的就是：**文本更新怎么做联邦聚合**。

**核心方法**

- 论文提出 `FedTextGrad`：
  - 每个客户端先用本地数据对 prompt 做 textual gradient 优化；
  - 然后把本地优化后的 prompt 上传给服务器；
  - 服务器不做数值平均，而是做 **文本汇总**。
- 难点在于，多个客户端各自改过的 prompt 里可能都有重要信息：
  - 直接让模型“总结一下”很容易把关键信息压丢；
  - 某些客户端特有但重要的修正规则，可能在汇总时被抹平。
- 为了解决这个问题，作者进一步改进 vanilla FedTextGrad：
  - 在服务器摘要提示里加入更明确的引导；
  - 借助 `Uniform Information Density` 的思路，提醒模型在汇总时尽量保留分布式更新里的高信息密度部分。
- 论文也系统讨论了本地步数等训练因素，说明 textual gradient 进入 FL 后，不是“把 prompt 优化套进去”就结束了，很多训练超参会明显影响结果。

**实验结论**

- 论文的核心结论不是“FedTextGrad 已经完全成熟”，而是“这条路可行，但聚合是瓶颈”。
- 作者通过实验确认：
  - textual gradient 确实可以在联邦场景工作；
  - 本地更新步数等设置很重要；
  - 服务器聚合如果处理不好，会明显损失各客户端学到的关键信息。
- 论文最重要的实证贡献，是把“文本更新如何聚合”单独拎成一个研究问题，并给出了比 vanilla 汇总更稳的改进方向。

**我怎么理解这篇论文**

- 如果说 APO 解决的是“单机 prompt 怎么自动变好”，FedTextGrad 解决的就是“多地 prompt 更新怎么合并成一个公共版本”。
- 它的新意不在于再发明一种 prompt optimizer，而在于把 textual gradient 真正带进了分布式优化语境。
- 这篇论文很值得看的一点是，它明确暴露出 textual gradient 的工程难题：文本不是向量，聚合不是求平均，信息保真会成为第一性问题。

### 4.4 MUSE-Autoskill: Self-Evolving Agents via Skill Creation, Memory, Management, and Evaluation
...
### 4.5 SkillForge: Forging Domain-Specific, Self-Evolving Agent Skills in Cloud Technical Support
...
### 4.6 SkillX: Automatically Constructing Skill Knowledge Bases for Agents
...
### 4.7 Trace2Skill: Distill Trajectory-Local Lessons into Transferable Agent Skills
...
### 4.8 SkillClaw: Let Skills Evolve Collectively with Agentic Evolver
...
### 4.9 Skill1: Unified Evolution of Skill-Augmented Agents via Reinforcement Learning
...
### 4.10 CoEvolve: Training LLM Agents via Agent-Data Mutual Evolution
...
### 4.11 Autogenesis: A Self-Evolving Agent Protocol
...
### 4.12 Agentic Harness Engineering: Observability-Driven Automatic Evolution of Coding-Agent Harnesses
...
### 4.13 Continual Harness: Online Adaptation for Self-Improving Foundation Agents
...
- **APO / FedTextGrad**：经验也可以先表现为一种**文本化优化信号**，目标是持续改写 prompt。
...
- **Prompt**：比 gene 更直接作用于模型输入，更新对象是文字指令本身及其改写规则。
...
- **APO**：minibatch 错误样本、自然语言批评、候选 prompt 评分。
- **FedTextGrad**：客户端 textual gradients、服务器汇总提示、聚合后的 prompt 效果。
...
- **Strategy Genes / APO / SkillX / Trace2Skill / Skill1** 更偏“在 benchmark 上看最终成绩”。
- **FedTextGrad** 除了看结果，还把“文本聚合是否丢信息”单独提升为主要研究问题。
- **MUSE** 已经把测试准入做成显式门禁。
...
2. **文本反馈也可以当优化信号。**  
   APO 和 FedTextGrad 说明，很多现实问题没有可微 loss，但仍然可以把“文字批评”组织成稳定的优化闭环。
...
7. **最终进化对象会从 skill 扩展到 harness 和协议资源。**  
...
- **APO / FedTextGrad**：再回答“如果没有数值梯度，prompt 能不能靠文本反馈持续变好，而且能否分布式聚合”。
...
`经验表示 -> Prompt 文本优化 -> 技能沉淀 -> 技能运维 -> 技能共享/联训 -> 数据分布进化 -> Harness / 协议级演化 -> 在线自适应`
...
3. [Can Textual Gradient Work in Federated Learning?](https://arxiv.org/abs/2502.19980)
4. [MUSE-Autoskill: Self-Evolving Agents via Skill Creation, Memory, Management, and Evaluation](https://arxiv.org/abs/2605.27366)
5. [SkillForge: Forging Domain-Specific, Self-Evolving Agent Skills in Cloud Technical Support](https://arxiv.org/abs/2604.08618)
6. [SkillX: Automatically Constructing Skill Knowledge Bases for Agents](https://arxiv.org/abs/2604.04804)
7. [Trace2Skill: Distill Trajectory-Local Lessons into Transferable Agent Skills](https://arxiv.org/abs/2603.25158)
8. [SkillClaw: Let Skills Evolve Collectively with Agentic Evolver](https://arxiv.org/abs/2604.08377)
9. [Skill1: Unified Evolution of Skill-Augmented Agents via Reinforcement Learning](https://arxiv.org/abs/2605.06130)
10. [CoEvolve: Training LLM Agents via Agent-Data Mutual Evolution](https://arxiv.org/abs/2604.15840)
11. [Autogenesis: A Self-Evolving Agent Protocol](https://arxiv.org/abs/2604.15034)
12. [Agentic Harness Engineering: Observability-Driven Automatic Evolution of Coding-Agent Harnesses](https://arxiv.org/abs/2604.25850)
13. [Continual Harness: Online Adaptation for Self-Improving Foundation Agents](https://arxiv.org/abs/2605.09998)
