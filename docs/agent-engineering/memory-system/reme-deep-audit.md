...
> 审计日期：2026-05-21
>
> **2026-05-30 验证更新**：核心架构描述（AgentScope 依赖、ReMeLight/ReMe 双系统、`pre_reasoning_hook` 四阶段管道、Turn 完整性保证、三级压缩、混合检索 0.7+0.3）已通过 `agentscope-ai/ReMe` 仓库与 README 复核为准确。此前对向量存储后端的判断过于保守：当前官方 README 明确宣称支持 `local / chroma / qdrant / elasticsearch / obvec / zvec / hologres` 等后端；而仓库目录中可以直接定位到的适配器以 `local / chroma / qdrant / elasticsearch` 为主。下文已按“代码可见实现 + README 公布能力面”两层口径修正。类继承链也已补充 `BaseMemory` 等中间抽象说明。
...
| 其他公开后端 | 官方 README 额外列出 Obvec / Zvec / Hologres；不同 release 的适配器落位可能不同 |
...
> **口径说明**：文件/向量后端在 README 与代码目录中的公开程度并不完全一致。本文优先把“目录中直接可定位的类”与“README 已明确承诺的支持面”区分开，避免把暂时未定位到的适配器误写成“不支持”。
