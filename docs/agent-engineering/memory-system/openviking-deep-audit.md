...
> 审计日期：2026-05-21
>
> **2026-05-30 验证更新**：本报告中的高层架构描述（VFS 范式、`viking://` URI、L0/L1/L2 分层、FastAPI + RAGFS、13 种 embedding 提供商）已通过 `volcengine/OpenViking` 仓库验证为准确。但部分具体类名（如 `HierarchicalRetriever`、`SemanticProcessor`、`SessionCompressor`、`MemoryExtractor`、`MemoryArchiver`）和 RAGFS 插件命名方案基于源码审计推测，可能与当前版本不完全一致。MCP 集成形态也已按当前公开文档修正：原生 `/mcp` 稳定公开面更接近 9 个工具，Claude/OpenClaw bot README 额外封装 7 个高阶工具；旧稿中的“15 个工具”计数已下调为历史汇总而非当前稳定承诺。
...
`openviking/server/mcp_endpoint.py`：当前公开文档中，更稳妥可证的原生 MCP tool surface 是 `/mcp` 端点提供的 **9 个工具**：
...
`bot/README.md` 中面向 Claude/OpenClaw 的 Agent 封装则额外公开了 7 个高阶工具：`search`、`read`、`remember`、`add_resource`、`health`、`grep`、`glob`。

> 注：旧稿中列出的 `code_outline` / `code_search` / `code_expand` / `forget` 等能力更像历史桥接层或插件侧能力，当前官方 README 没有把它们作为稳定原生 `/mcp` tool surface 明确承诺。
