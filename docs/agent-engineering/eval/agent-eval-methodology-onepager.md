...
本目录按“方法论 → 实施流程 → LLM Judge → 框架选型 → Benchmark → RAG 专项”阅读：

- [实施流程与报告模板](./agent.md)：把评测接入研发与发布流程。
- [LLM-as-a-Judge](./llm-as-a-judge.md)：评分协议、偏差控制与人工校准。
- [框架选型](./agent-eval-frameworks.md)：平台、库与 harness 的能力边界。
- [Benchmark 实现与选型](./agent-eval-benchmarks.md)：环境、验证器和可复现性。
- [RAG 评测](./rag.md)：检索、生成和端到端业务指标。

> 研究更新时间：2026-07-15。框架能力、基准版本和模型可用性变化很快；复现实验时应记录实际版本与运行配置。
